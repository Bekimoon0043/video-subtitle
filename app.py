...
def check_font():
    """Verify that the font file exists, log and raise if missing."""
    if not os.path.exists(FONT_PATH):
        ...
        try:
            ...
            if os.path.exists(fonts_dir):
                logger.info(f"Contents of {fonts_dir}: {os.listdir(fonts_dir)}")
        except Exception as e:
            logger.warning(f"Could not list fonts: {e}")
...
def add_text_overlay(input_path: str, output_path: str):
    """
    Run ffmpeg to overlay "Amharic Video" text.
    Returns output path on success, raises HTTPException on failure.
    """
    check_font()
